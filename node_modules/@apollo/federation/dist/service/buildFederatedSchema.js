...
function buildFederatedSchema(modulesOrSDL) {
    const modules = apollo_graphql_1.modulesFromSDL(modulesOrSDL);
    ...
    if (!schema.getQueryType()) {
        schema = new graphql_1.GraphQLSchema(Object.assign({}, schema.toConfig(), { query: new graphql_1.GraphQLObjectType({
                name: 'Query',
                fields: {},
            }) }));
    }
    ...
    schema = apollo_graphql_1.transformSchema(schema, type => {
        if (graphql_1.isObjectType(type) && type === schema.getQueryType()) {
            ...
            return new graphql_1.GraphQLObjectType(Object.assign({}, config, { fields: Object.assign({}, (hasEntities && { _entities: types_1.entitiesField }), { _service: Object.assign({}, types_1.serviceField, { resolve: () => ({ sdl }) }) }, config.fields) }));
        }
        return undefined;
    });
    schema = apollo_graphql_1.transformSchema(schema, type => {
        if (hasEntities && graphql_1.isUnionType(type) && type.name === types_1.EntityType.name) {
            return new graphql_1.GraphQLUnionType(Object.assign({}, types_1.EntityType.toConfig(), { types: entityTypes.filter(graphql_1.isObjectType) }));
        }
        return undefined;
    });
